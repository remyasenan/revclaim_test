...
module RoleRequirementSystem
  ...
  module RoleSecurityClassMethods
    ...
    #    require_role "admin", :only => :update, :unless => "current_remittor.authorized_for_listing?(params[:id]) "
    #
    # Valid options
    #
    ...
    # This is the core of RoleRequirement.  Here is where it discerns if a remittor can access a controller or not./
    def remittor_authorized_for?(remittor, params = {}, binding = self.binding)
      ...
      self.role_requirements.each{| role_requirement|
        ...
        passed = false
        roles.each { |role|
          passed = true if remittor.has_role?(role)
        } unless (remittor==:false || remittor==false)
  ...
  module RoleSecurityInstanceMethods
    ...
    def check_roles       
      return access_denied unless self.class.remittor_authorized_for?(current_remittor, params, binding)
      
      true
    end
    
  protected
    # receives a :controller, :action, and :params.  Finds the given controller and runs remittor_authorized_for? on it.
    ...
    def url_options_authenticate?(params = {})
      ...
      klass.remittor_authorized_for?(current_remittor, params, binding)
    end
  end
end